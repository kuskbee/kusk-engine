...
ComPtr<ID3D11DepthStencilState> maskDSS;		// 스텐실 버퍼에 표시
ComPtr<ID3D11DepthStencilState> drawMaskedDSS;	// 스텐실 표시된 곳만 그리기
...
void Graphics::InitRasterizerStates(ComPtr<ID3D11Device>& device) {
	...
	ThrowIfFailed(
		device->CreateRasterizerState(&rastDesc, postProcessingRS.GetAddressOf( )));
}
...
void Graphics::InitDepthStencilStates(ComPtr<ID3D11Device>& device) {
	...
	//* drawMaskedDSS : Stencil에 1로 표기된 경우에'만' 그리는 DSS
	...
	dsDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	dsDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL; // <- 유의
	dsDesc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
	dsDesc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
	dsDesc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
...
void Graphics::InitShaders(ComPtr<ID3D11Device>& device) {
	...
	vector<D3D11_INPUT_ELEMENT_DESC> samplingIEs = {
		{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
		{"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0},
		{"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 24, D3D11_INPUT_PER_VERTEX_DATA, 0},
	};

	vector<D3D11_INPUT_ELEMENT_DESC> skyboxIEs = {
		{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
		{"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0},
		{"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 24, D3D11_INPUT_PER_VERTEX_DATA, 0},
		{"TANGENT", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 32, D3D11_INPUT_PER_VERTEX_DATA, 0},
...
void Graphics::InitPipelineStates(ComPtr<ID3D11Device>& device) {
	...
	reflectSkyboxWirePSO.m_rasterizerState = wireCCWRS; // 반시계
